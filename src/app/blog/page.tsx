                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {featuredCategories.map((category) => (
                            <Link
                                ...
                                className={`bg-${category.color}-50 border border-${category.color}-200 rounded-lg p-6 hover:shadow-md transition-shadow`}
                            >
                                <h3 className={`text-${category.color}-800 font-semibold text-lg mb-2`}>
                                    {category.title}
                                </h3>
                ...
                <div className="flex flex-wrap gap-3">
                    {categories.map((category) => (
                        <Link
                            key={category._id}
                            href={`/categories/${category.slug.current}`}
                            className={`bg-${category.color}-100 text-${category.color}-800 px-4 py-2 rounded-full text-sm font-medium hover:bg-${category.color}-200 transition-colors`}
                        >
                            {category.title}
                        </Link>
                    ))}
                            ...
                            <div className="p-6">
                                ...
                                {post.category && (
                                    <Link
                                        ...
                                        className={`inline-block bg-${post.category.color}-100 text-${post.category.color}-800 px-3 py-1 rounded-full text-xs font-medium mb-3`}
                                    >
                                        {post.category.title}
                                    </Link>
                                )}
